<script lang="ts">
    ...
    onMount(async () => {
        const folderexists = await exists(
            await path.join(folder, "config.json"),
        );
        if (!folderexists) {
            await message(
                "config.jsonが見つかりません．\n 最初の画面に戻ります．",
    ...
    async function getthumbnail(game: Game) {
        const thumbnailpng = await path.join(
            folder,
            game.name,
            "thumbnail.png",
        );
        const fileexistspng = await exists(thumbnailpng);
        const thumbnailjpg = await path.join(
            folder,
            game.name,
            "thumbnail.jpg",
        );
    ...
    async function openGame(game: Game) {
        const gamefile = await path.join(
            folder,
            game.name,
            "game",
            game.gamefile,
        ...
        const response = await fetch(
            `http://gamecount.mersnn621.com/track?game=${game.id}`,
            {
                method: "GET",
            },
        );
...
<main
    class="font-line w-screen h-screen bg-[#eee] text-black flex flex-col items-center text-center transition-all duration-200"
>
    <div
        class="flex p-8 justify-between w-screen border-b-2 text-center items-center"
    >
        <p class="text-3xl select-none cursor-default">
            物理部展 2026 ゲーム展示
        </p>
        <div
            class="p-4 hover:bg-blue-600/10 rounded-xl transition duration-200 flex items-center gap-4"
        >
            <button
                onclick={() => {
                    window.location.href = "/";
                }}
                class="text-white/0 hover:text-blue-600 transition duration-300 cursor-pointer"
                >Go Home</button
            >
            <button
                onclick={() => {
    ...
    <div class="flex w-screen flex-1 justify-center">
        <div class="w-2/5 flex flex-col items-center justify-center p-16">
            {#each games as game}
                ...
                <div
                    class={`border-2 w-full text-left p-4 m-2 rounded-l-3xl flex flex-col transition-all duration-200  cursor-pointer ${selecting == game.id ? "translate-x-12 " : "hover:translate-x-4 hover:scale-105"}`}
                    onclick={() => {
                        selecting = game.id;
                    }}
                >
        ...
        <div class="w-3/5 flex flex-col justify-center items-center p-16">
            <div class="h-2/3 w-full flex flex-col gap-6 p-16">
                ...
                {:then thumbnail}
                    {#if thumbnail != ""}
                        <img
                            src={thumbnail}
                            alt=""
                            class="h-48 mx-auto rounded-xl object-cover"
                        />
                    {:else}
                        <div
                            class="h-48 bg-blue-700 w-96 mx-auto flex text-center"
                        >
                ...
                <p
                    class="text-left text-2xl h-68 overflow-x-scroll whitespace-pre-line hidden-scrollbar"
                >
            ...
            <div class="flex flex-col gap-8 items-center">
                <div
                    class={`border-2 border-amber-300 p-4 rounded-md ${selectinggame().caution ? "opacity-100" : "opacity-0"}`}
                >
                    このゲームには激しい点滅が含まれます．
                </div>
                <button
                    class="text-3xl p-4 border-2 rounded-md px-8 transition hover:scale-120 duration-200 w-fit"
                    onclick={() => {
